<?php
	if(!empty($_POST["baslik"]))
	{
		...
		if(($videoKontrol==1) or ($videoKontrol==2))
		{
			...
			if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				if((strtolower($format)==strtolower("a3p")) and (strtolower($pdfFormat)==strtolower("pdf")))	
				{		
					...
					{
						@include('login/inc/baglan.php');
						$sql="update alice set baslik='".trCevir($_POST["baslik"])."',PDF='$PDF',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='0' where id=".$_GET['aliceDuzenle'];
						$kontrol=1;
					}
					else
					...
					setcookie("bilgi","Desteklenen Formatlar &#8220; Konu Anlatým Dosyasý: pdf / Proje Dosyasý: a3p &#8221;");
			}
			else if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]==0))
			{
				...
				if(strtolower($format)==strtolower("a3p"))	
				{		
					if(!empty($alanlar["dosya"]))
					{
						$url="dosyalar/alice/".$alanlar["dosya"];
					...
					{
						@include('login/inc/baglan.php');
						$sql="update alice set baslik='".trCevir($_POST["baslik"])."',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='0' where id=".$_GET['aliceDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			{			
				...
				{		
					...
					{
						@include('login/inc/baglan.php');
						$sql="update alice set baslik='".trCevir($_POST["baslik"])."',PDF='$PDF',video='$kod',icerik='$_POST[icerik]',onay='0' where id=".$_GET['aliceDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			{
				@include('login/inc/baglan.php');
				$sql="update alice set baslik='".trCevir($_POST["baslik"])."',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['aliceDuzenle'];
				$kontrol=1;
			}
	...
	header("Location:index.php?alice&alice&aliceDuzenle=".$_GET['aliceDuzenle']);
?>
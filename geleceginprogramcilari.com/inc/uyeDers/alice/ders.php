<?php
	@mysql_query("ALTER TABLE alice ORDER BY id",$baglan);
	if(isset($_GET["devam"]))
	{
    	$ders=@mysql_fetch_array(mysql_query("SELECT * FROM alice where onay='1' and id='$_GET[devam]'" ));
		$ID=$_GET["devam"];
	}
	else
	{
		$ders=mysql_fetch_array(mysql_query("select * from alice where onay='1' order by id desc limit 0,1"));
		$ID=$ders["id"];
	}
	
	$puankontrol=0;
	$uyeId=@$_SESSION["$_SERVER[SERVER_NAME]uyeOturumID"];
	$sorgu=@mysql_query("select * from puanlama where ders='alice' and dersID='$ID' and yazarID='$uyeId'");
...
?>	
    ...
    <table border="0" style="float:left;width:400px;margin-top:-2px;">
        <tr>
            <td style="width:175px;">
                <form action="index.php?alice&puanKaydet&devam=<?php echo $ID; ?>" method="post" style="float:left;">
    ...
    <p style='margin:5px 0 0 -6px;font-size:13px;float:left;'><strong>Proje Dosyasý:</strong>
		<?php 
			if(!empty($ders["dosya"]))
				echo "$ders[dosya] - <a href='dosyalar/alice/$ders[dosya]'><strong>Ýndir</strong></a>"; 
        	else
				echo "Yok";
        ?>
    </p>
	...
	<a href="dosyalar/alice/<?php echo $ders['PDF']; ?>" target="_blank"><strong>týklayýnýz</strong>.</a></p>
    <?php } ?>
    <div class="clear"></div>
    ...
    <?php 
		...
		{ 
			...
			{
	?>
   				<iframe src="dosyalar/alice/<?php echo $ders['PDF']; ?>" style="width: 690px; height: 600px;" frameborder="0"></iframe>
    <?php }} ?>
    <?php 
		$browser_type = $_SERVER["HTTP_USER_AGENT"];

	...
	if(!empty($_POST["baslik"]))
	{
		...
		if(($videoKontrol==1) or ($videoKontrol==2))
		{
			...
			if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				if((strtolower($format)==strtolower("sb")) and (strtolower($pdfFormat)==strtolower("pdf")))	
				{		
					...
					{
						@include('login/inc/baglan.php');
						$sql="update smallbasic set baslik='".trCevir($_POST["baslik"])."',PDF='$PDF',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='0' where id=".$_GET['arduinoDuzenle'];
						$kontrol=1;
					}
					else
					...
					setcookie("bilgi","Desteklenen Formatlar &#8220; Konu Anlatým Dosyasý: pdf / Proje Dosyasý: sb &#8221;");
			}
			else if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]==0))
			{
				...
				if(strtolower($format)==strtolower("sb"))	
				{		
					if(!empty($alanlar["dosya"]))
					{
						$url="dosyalar/smallBasic/".$alanlar["dosya"];
					...
					{
						@include('login/inc/baglan.php');
						$sql="update smallbasic set baslik='".trCevir($_POST["baslik"])."',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='0' where id=".$_GET['arduinoDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			else if((@$_FILES["dosya"]["size"]==0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				if(strtolower($pdfFormat)==strtolower("pdf"))	
				{
					...
					{
						@include('login/inc/baglan.php');
						$sql="update smallbasic set baslik='".trCevir($_POST["baslik"])."',PDF='$PDF',video='$kod',icerik='$_POST[icerik]',onay='0' where id=".$_GET['arduinoDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			{
				@include('login/inc/baglan.php');
				$sql="update smallbasic set baslik='".trCevir($_POST["baslik"])."',video='$kod',icerik='$_POST[icerik]',onay='0' where id=".$_GET['arduinoDuzenle'];
				$kontrol=1;
			}

		...
		if(($videoKontrol==1) or ($videoKontrol==2))
		{
			...
			if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				if ($format == end($dosyaAdi))
					...
					$dosyaYolu="../dosyalar/smallBasic/".$dosya;
				}
				...
				if ($pdfFormat == end($pdfAdi))
					...
					$pdfYolu="../dosyalar/smallBasic/".$PDF;
				}
				
				if((strtolower($format)==strtolower("sb") or strtolower($format)==strtolower("rar")) and (strtolower($pdfFormat)==strtolower("pdf")))	
				{		
					...
					{
						$sql="update smallbasic set baslik='$_POST[baslik]',PDF='$PDF',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['smallBasicDuzenle'];
						$kontrol=1;
					}
					else
					...
					setcookie("bilgi","Desteklenen Formatlar [ Konu Anlatým Dosyasý: pdf / Proje Dosyasý: sb - rar ]");
			}
			else if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]==0))
			{
				...
				if(strtolower($format)==strtolower("sb") or strtolower($format)==strtolower("rar"))	
				{		
					if(!empty($alanlar["dosya"]))
					{
						$url="../dosyalar/smallBasic/".$alanlar["dosya"];
					...
					{
						$sql="update smallbasic set baslik='$_POST[baslik]',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['smallBasicDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			else if((@$_FILES["dosya"]["size"]==0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				if(strtolower($pdfFormat)==strtolower("pdf"))	
				{
					...
					{
						$sql="update smallbasic set baslik='$_POST[baslik]',PDF='$PDF',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['smallBasicDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			{
				$sql="update smallbasic set baslik='$_POST[baslik]',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['smallBasicDuzenle'];
				$kontrol=1;
			}

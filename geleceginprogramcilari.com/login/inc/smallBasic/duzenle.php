<?php
	...
	if((isset($_GET["smallBasicDuzenle"])) and (isset($_POST["baslik"])))
		@include('inc/smallBasic/adminGuncelle.php');
	if((isset($_GET["smallBasicDuzenle"])) and (isset($_GET["dosyaSil"])))
	{
		
		$sorgu=mysql_query("select * from smallbasic where id=".$_GET['smallBasicDuzenle']);
		...
		if(!empty($dosya["dosya"]))
		{
			...
			if(unlink("$url"))
			{
				$sql="update smallbasic set dosya='' where id=".$_GET['smallBasicDuzenle'];
				if(@mysql_query($sql))
		...
		header("Location:index.php?smallBasicDuzenle=".$_GET['smallBasicDuzenle']);
	}
	if((isset($_GET["smallBasicDuzenle"])) and (isset($_GET["PDFSil"])))
	{
		...
		if(!empty($dosya["PDF"]))
		{
			...
			if(unlink("$url"))
			{
				$sql="update smallbasic set PDF='' where id=".$_GET['smallBasicDuzenle'];
				if(@mysql_query($sql))
					setcookie("bilgi","Dosya silindi.");
				else
					setcookie("bilgi","Silme iþlemi yapýlmadý.");
	...
	$sql=@mysql_query("select * from smallbasic where id=".$_GET['smallBasicDuzenle']);
	$alanlar=mysql_fetch_array($sql);
	?>
    <form method="post" target="_self" action="index.php?smallBasicDuzenle=<?php echo $_GET['smallBasicDuzenle']; ?>" enctype="multipart/form-data">
        ...
        <table width="100%" border="0" style="margin:-3px;">
          <tr>
            ...
            <td style="width:80px;"> <a class="Button" style="float:left; margin:3px 0 0 0; width:75px;text-align:center;" href="index.php?PDFSil&smallBasicDuzenle=<?php echo $_GET['smallBasicDuzenle']; ?>">Dosyayý Sil</a></td>
            <td style="width:37px;"> 
            	<a class="Button" style="float:left; margin:3px 0 0 0;" href="../dosyalar/smallBasic/<?php echo $alanlar['PDF']; ?>" target="_blank">Ýndir</a>
            </td>
			<?php } ?>
        ...
        <h5>Proje dosyasýný buradan y&uuml;kleyiniz. &raquo; Formatlar: sb - rar</h5>
        <table width="100%" border="0" style="margin:-3px;">
          <tr>
            ...
            <td style="width:80px;"> <a class="Button" style="float:left; margin:3px 0 0 0; width:75px;text-align:center;" href="index.php?dosyaSil&smallBasicDuzenle=<?php echo $_GET['smallBasicDuzenle']; ?>">Dosyayý Sil</a></td>
            <td style="width:37px;"> 
            	<a class="Button" style="float:left; margin:3px 0 0 0;" href="../dosyalar/smallBasic/<?php echo $alanlar['dosya']; ?>" target="_blank">Ýndir</a>
            </td>
			<?php } ?>

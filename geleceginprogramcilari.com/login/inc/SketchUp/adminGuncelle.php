<?php
	if(!empty($_POST["baslik"]))
	{
		...
		if(($videoKontrol==1) or ($videoKontrol==2))
		{
			...
			if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				if((strtolower($format)==strtolower("rar") or strtolower($format)==strtolower("skp")) and (strtolower($pdfFormat)==strtolower("pdf")))	
				{		
					...
					{
						$sql="update sketchup set baslik='$_POST[baslik]',PDF='$PDF',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['SketchUpDuzenle'];
						$kontrol=1;
					}
					else
					...
					setcookie("bilgi","Desteklenen Formatlar [ Konu Anlatým Dosyasý: pdf / Proje Dosyasý: skp - rar ]");
			}
			else if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]==0))
			{
				...
				if(strtolower($format)==strtolower("rar") or strtolower($format)==strtolower("skp"))	
				{		
					if(!empty($alanlar["dosya"]))
					{
						$url="../dosyalar/sketchup/".$alanlar["dosya"];
					...
					{
						$sql="update sketchup set baslik='$_POST[baslik]',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['SketchUpDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			else if((@$_FILES["dosya"]["size"]==0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				{		
					...
					{
						$sql="update sketchup set baslik='$_POST[baslik]',PDF='$PDF',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['SketchUpDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			{
				$sql="update sketchup set baslik='$_POST[baslik]',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['SketchUpDuzenle'];
				$kontrol=1;
			}
	...
	header("Location:index.php?SketchUpDuzenle=".$_GET['SketchUpDuzenle']);
?>
<?php
	if(!empty($_POST["baslik"]))
	{
		...
		if(($videoKontrol==1) or ($videoKontrol==2))
		{
			...
			if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				if((strtolower($format)==strtolower("aia") or strtolower($format)==strtolower("apk") or strtolower($format)==strtolower("rar")) and (strtolower($pdfFormat)==strtolower("pdf")))	
				{			
					if((@copy($_FILES['dosya']['tmp_name'],$dosyaYolu)) and (@copy($_FILES['PDF']['tmp_name'],$pdfYolu)))
					{
						$sql="insert into app (baslik,PDF,dosya,video,icerik,yazarID,onay) values ('$_POST[baslik]','$PDF','$dosya','$kod','$_POST[icerik]','','1')";
						$kontrol=1;
					}
					else
			...
			else if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]==0))
			{
				...
				if(strtolower($format)==strtolower("aia") or strtolower($format)==strtolower("apk") or strtolower($format)==strtolower("rar"))	
				{			
	...
	header("Location:index.php?appDersler");
?>
		...
		if(($videoKontrol==1) or ($videoKontrol==2))
		{
			...
			if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				if ($pdfFormat == end($pdfAdi))
					...
					$pdfYolu="../dosyalar/app/".$PDF;
				}
				
				if((strtolower($format)==strtolower("rar") or strtolower($format)==strtolower("aia") or strtolower($format)==strtolower("apk")) and (strtolower($pdfFormat)==strtolower("pdf")))	
				{		
					...
					{
						$sql="update app set baslik='$_POST[baslik]',PDF='$PDF',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['appUyeDuzenle'];
						$kontrol=1;
					}
					else
					...
					setcookie("bilgi","Desteklenen Formatlar [ Konu Anlatým Dosyasý: pdf / Proje Dosyasý: aia - apk - rar ]");
			}
			else if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]==0))
			{
				...
				if(strtolower($format)==strtolower("rar") or strtolower($format)==strtolower("aia") or strtolower($format)==strtolower("apk"))	
				{		
					if(!empty($alanlar["dosya"]))
					{
						$url="../dosyalar/app/".$alanlar["dosya"];
					...
					{
						$sql="update app set baslik='$_POST[baslik]',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['appUyeDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			else if((@$_FILES["dosya"]["size"]==0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				{		
					...
					{
						$sql="update app set baslik='$_POST[baslik]',PDF='$PDF',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['appUyeDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			{
				$sql="update app set baslik='$_POST[baslik]',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['appUyeDuzenle'];
				$kontrol=1;
			}

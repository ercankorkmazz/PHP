<?php
	...
	if((isset($_GET["aliceUyeDuzenle"])) and (isset($_POST["baslik"])))
		@include('inc/alice/uye/Guncelle.php');
	if((isset($_GET["aliceUyeDuzenle"])) and (isset($_GET["dosyaSil"])))
	{
		
		$sorgu=mysql_query("select * from alice where id=".$_GET['aliceUyeDuzenle']);
		...
		if(!empty($dosya["dosya"]))
		{
			...
			if(unlink("$url"))
			{
				$sql="update alice set dosya='' where id=".$_GET['aliceUyeDuzenle'];
				if(@mysql_query($sql))
		...
		header("Location:index.php?aliceUyeDuzenle=".$_GET['aliceUyeDuzenle']);
	}
	if((isset($_GET["aliceUyeDuzenle"])) and (isset($_GET["PDFSil"])))
	{
		...
		if(!empty($dosya["PDF"]))
		{
			...
			if(unlink("$url"))
			{
				$sql="update alice set PDF='' where id=".$_GET['aliceUyeDuzenle'];
				if(@mysql_query($sql))
					setcookie("bilgi","Dosya silindi.");
				else
					setcookie("bilgi","Silme iþlemi yapýlmadý.");
	...
	$sql=@mysql_query("select * from alice where id=".$_GET['aliceUyeDuzenle']);
	$alanlar=mysql_fetch_array($sql);
	?>
    <form method="post" target="_self" action="index.php?aliceUyeDuzenle=<?php echo $_GET['aliceUyeDuzenle']; ?>" enctype="multipart/form-data">
        ...
        <table width="100%" border="0" style="margin:-3px;">
          <tr>
            ...
            <td style="width:80px;"> <a class="Button" style="float:left; margin:3px 0 0 0; width:75px;text-align:center;" href="index.php?PDFSil&aliceUyeDuzenle=<?php echo $_GET['aliceUyeDuzenle']; ?>">Dosyayý Sil</a></td>
            <td style="width:37px;"> <a class="Button" style="float:left; margin:3px 0 0 0;" href="../dosyalar/alice/<?php echo $alanlar['PDF']; ?>" target="_blank">Ýndir</a></td>
			<?php } ?>
        ...
        <table width="100%" border="0" style="margin:-3px;">
          <tr>
            ...
            <td style="width:80px;"> <a class="Button" style="float:left; margin:3px 0 0 0; width:75px;text-align:center;" href="index.php?dosyaSil&aliceUyeDuzenle=<?php echo $_GET['aliceUyeDuzenle']; ?>">Dosyayý Sil</a></td>

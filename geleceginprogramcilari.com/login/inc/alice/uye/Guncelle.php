		...
		if(($videoKontrol==1) or ($videoKontrol==2))
		{
			...
			if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				if ($format == end($dosyaAdi))
					...
					$dosyaYolu="../dosyalar/alice/".$dosya;
				}
				...
				if ($pdfFormat == end($pdfAdi))
					...
					$pdfYolu="../dosyalar/alice/".$PDF;
				}
				
				if((strtolower($format)==strtolower("rar") or strtolower($format)==strtolower("a3p")) and (strtolower($pdfFormat)==strtolower("pdf")))	
				{		
					...
					{
						$sql="update alice set baslik='$_POST[baslik]',PDF='$PDF',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['aliceUyeDuzenle'];
						$kontrol=1;
					}
					else
					...
					setcookie("bilgi","Desteklenen Formatlar [ Konu Anlatým Dosyasý: pdf / Proje Dosyasý: a3p - rar ]");
			}
			else if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]==0))
			{
				...
				if(strtolower($format)==strtolower("rar") or strtolower($format)==strtolower("a3p"))	
				{		
					if(!empty($alanlar["dosya"]))
					{
						$url="../dosyalar/alice/".$alanlar["dosya"];
					...
					{
						$sql="update alice set baslik='$_POST[baslik]',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['aliceUyeDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			else if((@$_FILES["dosya"]["size"]==0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				if(strtolower($pdfFormat)==strtolower("pdf"))	
				{
					...
					{
						$sql="update alice set baslik='$_POST[baslik]',PDF='$PDF',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['aliceUyeDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			{
				$sql="update alice set baslik='$_POST[baslik]',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['aliceUyeDuzenle'];
				$kontrol=1;
			}

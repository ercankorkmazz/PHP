		...
		if(($videoKontrol==1) or ($videoKontrol==2))
		{
			...
			if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				if ($pdfFormat == end($pdfAdi))
					...
					$pdfYolu="../dosyalar/kodu/".$PDF;
				}
				
				if((strtolower($format)==strtolower("rar") or strtolower($format)==strtolower("kodu") or strtolower($format)==strtolower("kodu2")) and (strtolower($pdfFormat)==strtolower("pdf")))	
				{		
					...
					{
						$sql="update kodu set baslik='$_POST[baslik]',PDF='$PDF',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['koduUyeDuzenle'];
						$kontrol=1;
					}
					else
					...
					setcookie("bilgi","Desteklenen Formatlar [ Konu Anlatým Dosyasý: pdf / Proje Dosyasý: kodu - kodu2 - rar ]");
			}
			else if((@$_FILES["dosya"]["size"]!=0) and (@$_FILES["PDF"]["size"]==0))
			{
				...
				if(strtolower($format)==strtolower("rar") or strtolower($format)==strtolower("kodu") or strtolower($format)==strtolower("kodu2"))	
				{		
					if(!empty($alanlar["dosya"]))
					{
						$url="../dosyalar/kodu/".$alanlar["dosya"];
					...
					{
						$sql="update kodu set baslik='$_POST[baslik]',dosya='$dosya',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['koduUyeDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			else if((@$_FILES["dosya"]["size"]==0) and (@$_FILES["PDF"]["size"]!=0))
			{
				...
				{		
					...
					{
						$sql="update kodu set baslik='$_POST[baslik]',PDF='$PDF',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['koduUyeDuzenle'];
						$kontrol=1;
					}
					else
						setcookie("bilgi","Dosya Yüklenemedi!");
			...
			{
				$sql="update kodu set baslik='$_POST[baslik]',video='$kod',icerik='$_POST[icerik]',onay='$onay' where id=".$_GET['koduUyeDuzenle'];
				$kontrol=1;
			}

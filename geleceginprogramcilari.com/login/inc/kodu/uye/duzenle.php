<?php
	...
	if((isset($_GET["koduUyeDuzenle"])) and (isset($_POST["baslik"])))
		@include('inc/kodu/uye/Guncelle.php');
	if((isset($_GET["koduUyeDuzenle"])) and (isset($_GET["dosyaSil"])))
	{
		
		$sorgu=mysql_query("select * from kodu where id=".$_GET['koduUyeDuzenle']);
		...
		if(!empty($dosya["dosya"]))
		{
			...
			if(unlink("$url"))
			{
				$sql="update kodu set dosya='' where id=".$_GET['koduUyeDuzenle'];
				if(@mysql_query($sql))
		...
		header("Location:index.php?koduUyeDuzenle=".$_GET['koduUyeDuzenle']);
	}
	if((isset($_GET["koduUyeDuzenle"])) and (isset($_GET["PDFSil"])))
	{
		...
		if(!empty($dosya["PDF"]))
		{
			...
			if(unlink("$url"))
			{
				$sql="update kodu set PDF='' where id=".$_GET['koduUyeDuzenle'];
				if(@mysql_query($sql))
					setcookie("bilgi","Dosya silindi.");
				else
					setcookie("bilgi","Silme iþlemi yapýlmadý.");
	...
	$sql=@mysql_query("select * from kodu where id=".$_GET['koduUyeDuzenle']);
	$alanlar=mysql_fetch_array($sql);
	?>
    <form method="post" target="_self" action="index.php?koduUyeDuzenle=<?php echo $_GET['koduUyeDuzenle']; ?>" enctype="multipart/form-data">
        ...
        <table width="100%" border="0" style="margin:-3px;">
          <tr>
            ...
            <td style="width:80px;"> <a class="Button" style="float:left; margin:3px 0 0 0; width:75px;text-align:center;" href="index.php?PDFSil&koduUyeDuzenle=<?php echo $_GET['koduUyeDuzenle']; ?>">Dosyayý Sil</a></td>
            <td style="width:37px;"> <a class="Button" style="float:left; margin:3px 0 0 0px;" href="../dosyalar/kodu/<?php echo $alanlar['PDF']; ?>" target="_blank">Ýndir</a></td>
			<?php } ?>
            <td> <span style="color:#FFF;"> <strong>Dosya: </strong><?php if(!empty($alanlar['PDF']))echo $alanlar['PDF']; else echo "Yok"; ?></span></td>
          </tr>
		</table>
        <h5>Proje dosyasýný buradan y&uuml;kleyiniz. &raquo; Formatlar: kodu - kodu2 - rar</h5>
        <table width="100%" border="0" style="margin:-3px;">
          <tr>
            ...
            <td style="width:80px;"> <a class="Button" style="float:left; margin:3px 0 0 0; width:75px;text-align:center;" href="index.php?dosyaSil&koduUyeDuzenle=<?php echo $_GET['koduUyeDuzenle']; ?>">Dosyayý Sil</a></td>
            <td style="width:37px;"> <a class="Button" style="float:left; margin:3px 0 0 0px;" href="../dosyalar/kodu/<?php echo $alanlar['dosya']; ?>" target="_blank">Ýndir</a></td>
			<?php } ?>
